...
class AirportOpsEnv:
    ...
    def _score_crisis_protocol(self, action: dict) -> float:
        current_step = self.state_machine.state.step
        active_crises = self.event_manager.get_active_crises(current_step)
        ...
        scores = []
        for crisis in active_crises:
            score, missing = self.event_manager.check_protocol_compliance(action, crisis)
            scores.append(score)
