...
class AirportStateMachine:
    ...
    def score_eta_optimality(
        self, runway_id: str, gate_id: str, flight_type: str = "commercial"
    ) -> float:
        """
    ...
    def _load_scenario(self, scenario: dict) -> None:
        ...
        for r in scenario["runways"]:
            status_map = {
                "maintenance": RunwayStatus.MAINTENANCE,
                "occupied": RunwayStatus.OCCUPIED,
                "closed": RunwayStatus.CLOSED,
                "free": RunwayStatus.FREE,
                "emergency": RunwayStatus.EMERGENCY,
            }
        ...
        for g in scenario["gates"]:
            gate_status_map = {
                "occupied": GateStatus.OCCUPIED,
                "reserved": GateStatus.RESERVED,
                "free": GateStatus.FREE,
            }
            self.state.gates[g["id"]] = Gate(
                id=g["id"],
                type=GateType(g["type"]),
                status=gate_status_map.get(g.get("status", "free"), GateStatus.FREE),
            )
